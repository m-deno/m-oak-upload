// 文件上传中间件
import { FormDataFile, join, ensureDirSync, copySync } from "./deps.ts";
...
// 过滤文件上传key
function filterFileKeys(
  files: string[] | undefined,
  fileList: FormDataFile[]
): FormDataFile[] {
  if (!files) {
    return fileList;
  }
  ...
  files.forEach((name) => {
    const foundedFile = fileList.find(
      (file: FormDataFile) => file.name === name
    );
    if (foundedFile) {
...
// 处理最大文件数
function filterFileMaxNumber(
  maxFileNumber: number | undefined,
  fileList: FormDataFile[]
) {
...
export function upload(path = "uploads", options: IOptions = defaultIOptions) {
  return async (ctx: any, next: CallableFunction) => {
    ...
    if (!contentType || !contentType.match(boundaryRegex)) {
      ctx.throw(422, "请求类型错误，请确认请求类型为multipart/form-data");
    }
    // 从body value中读取表单数据
    const body = await ctx.request.body({ type: "form-data" });
    const formData = await body.value.read();
    if (!formData.files) {
      ctx.throw(422, "当前请求未上传任何文件");
    }
    // 结果文件列表
    ...
    // 筛选文件name, 上传时的key
    const foundedFiles: FormDataFile[] = filterFileKeys(
      options.files,
      fileList
    );
    ...
    // 处理扩展名
    const foundedFileExts: FormDataFile[] = filterFileExts(
      options.exts,
      fileList
    );
    ...
    // 处理文件最大数量
    const foundedFileMaxNumbers: FormDataFile[] = filterFileMaxNumber(
      options.maxFile,
      fileList
    );
    ...
    for (const file of fileList) {
      ...
      // 缓存文件存在会同步到上传目录
      if (fileObj.tmpUrl) {
        copySync(fileObj.tmpUrl, fileObj.path);
        Deno.removeSync(fileObj.tmpUrl);
      }
